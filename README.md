...
Impact visuals intentionally avoid arcade-style expanding shockwave rings, fake particles, or decorative ejecta streaks. Feedback is limited to crater/damage marks, body heat tint, changed motion, and real simulated fragment bodies. Catastrophic impacts now strip chunks progressively from the original body, leave the damaged main remnant alive, and use strict collision/debris budgets so one impact cannot freeze the browser or shred the same body repeatedly in a single moment. For browser performance, fragments are lightweight debris: they are pulled by major bodies but do not mutually gravitate with every other fragment.
